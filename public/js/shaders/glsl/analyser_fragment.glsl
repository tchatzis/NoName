...
void main()
{
	...
	float level = vPosition.y / vLevel + 0.5;
	float red = ( vLevel - vPosition.y ) / vLevel;
	float fade = vAlternate.g * max( duration - time, 0.0 );

	if ( level < red )
		color.g += vColor.g + fade;

	if ( level > 0.5 )
		color.r += vColor.r + fade;
